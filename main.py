...
def train(model,
    ...
    for epoch in range(num_epochs):
        for (Id,label,i11,i12,i13,i14,i15,i01,i02,i03,i04,i05), _ in train_loader:
            ...
            x11 = read_memory(i11,memory_bank)
            x12 = read_memory(i12,memory_bank)
            x13 = read_memory(i13,memory_bank)
            x14 = read_memory(i14,memory_bank)
            x15 = read_memory(i15,memory_bank)
            x01 = read_memory(i01,memory_bank)
            x02 = read_memory(i02,memory_bank)
            x03 = read_memory(i03,memory_bank)
            x04 = read_memory(i04,memory_bank)
            x05 = read_memory(i05,memory_bank)
            
            sim11 = (torch.cosine_similarity(x, x11, dim=1)+1)/2
            sim12 = (torch.cosine_similarity(x, x12, dim=1)+1)/2
            sim13 = (torch.cosine_similarity(x, x13, dim=1)+1)/2
            sim14 = (torch.cosine_similarity(x, x14, dim=1)+1)/2
            sim15 = (torch.cosine_similarity(x, x15, dim=1)+1)/2
            sim01 = (torch.cosine_similarity(x, x01, dim=1)+1)/2
            sim02 = (torch.cosine_similarity(x, x02, dim=1)+1)/2
            sim03 = (torch.cosine_similarity(x, x03, dim=1)+1)/2
            sim04 = (torch.cosine_similarity(x, x04, dim=1)+1)/2
            sim05 = (torch.cosine_similarity(x, x05, dim=1)+1)/2
            sim1 = (sim11 + sim12 + sim13 + sim14 + sim15 )/5
            sim0 = (sim01 + sim02 + sim03 + sim04 + sim05 )/5
            sim = (sim0 - sim1)/2
